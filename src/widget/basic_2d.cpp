...
uiLabel::uiLabel(const std::string text_, const Vector3b& col_) {
  text = text_;
  col = col_;
}
void uiLabel::render() {
...
template <typename T> uiSliderH<T>::uiSliderH(const std::string& text_, T* value_, const Vector2& range_, const double delta_) {
  text = text_;
  value = value_;
  range = range_;
  delta = delta_;
...
template <typename T> bool uiSliderH<T>::CallbackFunc(uiCallbackFlags flag, Vector2d vec2_1, [[maybe_unused]] int num_1, [[maybe_unused]] int num_2, [[maybe_unused]] const char** strings) {
...
template <typename T> uiRange<T>::uiRange(const std::string& text_, _Vec<T, 2>* value_, const Vector2& range_, const double delta_) {
  text = text_;
  value = value_;
  range = range_;
  delta = delta_;
...
template <typename T> void uiRange<T>::render() {
  if(!flags.Active) {
    needRendering(false);
    return;
  }
...
template <typename T> bool uiRange<T>::CallbackFunc(uiCallbackFlags flag, Vector2d vec2_1, [[maybe_unused]] int num_1, [[maybe_unused]] int num_2, [[maybe_unused]] const char** strings) {
  ...
  if(flag == uiCallbackFlags::LMouseDown || flag == uiCallbackFlags::MouseScroll) {
    ...
    if(std::abs(vec2_1[0] - p0) < th)
      selecting_idx = 0;
    else if(std::abs(vec2_1[0] - p1) < th)
      selecting_idx = 1;
    else
      selecting_idx = -1;
  }
...
void uiMultiLine::operator<<(const std::string& str) {
  text += str;
  needRendering(true);
}
void uiMultiLine::operator<<(const char* str) {
  text += std::string(str);
  needRendering(true);
}
void uiMultiLine::operator<<(const int val) {
  text += std::to_string(val);
  needRendering(true);
}
void uiMultiLine::operator<<(const double val) {
  text += std::to_string(val);
  needRendering(true);
}
...
template <typename T> uiVector3<T>::uiVector3(std::string text_, _Vec<T, 3>* value_, Vector2 range_, const double delta_) {
  text = text_;
  value = value_;
  range = range_;
  delta = delta_;
...
template <typename T> void uiVector3<T>::render() {
  if(!flags.Active) {
    needRendering(false);
    std::cout << "Not visivle or not active widget\n";
    return;
...
template <typename T> bool uiVector3<T>::CallbackFunc(uiCallbackFlags flag, Vector2d vec2_1, [[maybe_unused]] int num_1, [[maybe_unused]] int num_2, [[maybe_unused]] const char** strings) {
...
void uiFrame::render() {
  ...
  if(flags.needRendering > 0) {
    ...
    wnd->AddTriangle2D(pos + size - Vector2d{resizer_size, 0}, pos + size - Vector2d{0, 0}, pos + size - Vector2d{0, resizer_size}, Vector3b{100, 100, 200});
...
void uiTextTexture::render() {
  ...
  if(flags.needRendering > 0) {
    ...
    wnd->AddRectPosSize(pos, size, style->col_WidgetLine, 2);
    /* const zoom_size = 100; */
    wnd->AddRectPosSize({pos[0], pos[1] + title_bar}, {size[0], size[1] - title_bar}, {0, 0}, {fw, fh});
...
bool uiTextTexture::CallbackFunc(uiCallbackFlags flag, Vector2d vec2_1, [[maybe_unused]] int num_1, [[maybe_unused]] int num_2, [[maybe_unused]] const char** strings) {
  if(!flags.Active) return false;
  CallbackTitlebar(flag, vec2_1, num_1, num_2, strings);
  CallbackResizer(flag, vec2_1, num_1, num_2, strings);
  updateScrollPos();
...
void uiWidget::renderScrollbars() {
  ...
  if(innerSize[1] > size[1] && flags.needRendering > 0) {
    ...
    wnd->AddRectPosSize(Vector2d(pos_scrollY[0] + 1, pos[1] + scrollPos[1] * size[1] / innerSize[1]), Vector2d(style->ScrollbarWidth - 1, (size[1] - style->ScrollbarWidth) * size[1] / innerSize[1]),
                        style->col_wndScroll);
  }
  ...
  if(innerSize[0] > size[0] && flags.needRendering > 0) {
    ...
    wnd->AddRectPosSize(Vector2d(pos[0] + scrollPos[0] * size[0] / innerSize[0], pos_scrollX[1] + 1), Vector2d(size[0] * (size[0] - style->ScrollbarWidth) / innerSize[0], style->ScrollbarWidth - 1),
                        style->col_wndScroll);
  }
...
void uiWidget::CallbackTitlebar(uiCallbackFlags flag, Vector2d vec2_1, [[maybe_unused]] int num_1, [[maybe_unused]] int num_2, [[maybe_unused]] const char** strings) {
  ...
  if(flag == uiCallbackFlags::MouseMove && is_moving && (num_2 & 0x01)) {
    ...
    if(touching_closebtn) {
      ...
      if(parent->align_type == uiWidgetAlignTypes::Absolute && !parent->isRenderedTop(this))
        parent->toRenderTop(this);
      else
        CollapseTitlebar(!flags.CollapsingTitlebar);
    }
...
bool uiWidget::CallbackScrollbars(uiCallbackFlags flag, Vector2d vec2_1, [[maybe_unused]] int num_1, [[maybe_unused]] int num_2, [[maybe_unused]] const char** strings) {
  ...
  if(innerSize[1] >= size[1]) {
    isContains = (pos[0] + size[0] - style->ScrollbarWidth <= vec2_1[0]) && (vec2_1[0] <= pos[0] + size[0]) && (pos[1] <= vec2_1[1]) && (vec2_1[1] < pos[1] + size[1] - 30);
