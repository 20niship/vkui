...
std::string myFormat(const std::string &base_str, const Args ...args ){
...
void uiWindow::createCommandBuffers(bool force = false) {
    ...
    if(last_vertex_size == vertices.size() && last_vertex_ui_size == vertices_ui.size()  &&  !force) return;

    last_vertex_size = vertices.size();
    last_vertex_ui_size = vertices_ui.size();
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
        ...
        commandBuffers[i].draw(static_cast<uint32_t>(vertices_ui.size()), 1, 0, 0);
        // for(int i=0; i<vertices_ui.size(); i++){
        //     std::cout << vertices_ui[i].pos[0] << ", " << vertices_ui[i].pos[1] << std::endl;
        // }
...
void uiWindow::renderUI(){
    ...
    // if(root_widget_ui.getNeedRendering()){
        ...
        vertices_ui.resize(0);
        root_widget_ui.render_child_widget();
    // }
    if(vertices_ui.size() == 0){
        vertices_ui.push_back(VertexUI(Vector2{0, 0}, Vector3b{0, 0, 0},Vector2{0, 0}));        
        vertices_ui.push_back(VertexUI(Vector2{0, 0}, Vector3b{0, 0, 0},Vector2{0, 0}));        
        vertices_ui.push_back(VertexUI(Vector2{0, 0}, Vector3b{0, 0, 0},Vector2{0, 0}));        
    }
}
...
void uiWindow::updateVertexBuffer(){
    ...
    if(!_created_vertex_buffer){
        ...
        const vk::DeviceSize bufferSize2 = vertices_ui.size_in_bytes();
...
#endif
    for(int i=0; i<16; i++){ uniform_data.proj_uv[i] = projectionMatrix[i]; }
...
#endif
        "vert_size = ({}, {}), drawCmd={} nWidget={}, fps={}", 
        vertices.size(), vertices_ui.size(), nCmd, wsize, fps
    );
...
void uiWindow::drawFrame(const bool verbose) {
    ...
    vertexBuffer_ui.copyData((void *)vertices_ui.data(), vertices_ui.size_in_bytes());
    if(verbose){ 
      std::cout << "vertex size(3d, 2d, 2d_overlay) = " << vertices.size() << ", " << vertices_ui.size() << std::endl;
    }
    ...
    try {
        auto result = (*device)->acquireNextImageKHR(swapChain, std::numeric_limits<uint64_t>::max(),
            (*imageAvailableSemaphores)[currentFrame_2], nullptr);
...
void uiWindow::AddLine(const Vector3 &pos1, const Vector3 &pos2, const Vector3b &col1, const Vector3b &col2, const float width){
    auto dp = pos2 - pos1; dp = dp / dp.norm();
    Vector3 e1, e2;
...
void uiWindow::AddPoint_mono_triangle(const Vector3 &pos, const Vector3b &col, const double size){
    AddTriangle(
        pos + Vector3({size, 0.0f, 0.0f}),
        pos + Vector3({0.0f, 0.0f, size}),
        pos + Vector3({0.0f, size, 0.0f}), col);
    AddTriangle(
        pos + Vector3({0.0f, 0.0f, size}),
        pos + Vector3({size, 0.0f, 0.0f}),
        pos + Vector3({0.0f, size, 0.0f}), col);
...
void uiWindow::AddCube(const Vector3& pos, const Vector3& whd, const Vector3b& col){
    ...
    const Vector3 points[8] = {
        pos +  Vector3(-half_size[0], -half_size[1], -half_size[2]),
        pos +  Vector3(-half_size[0], -half_size[1],  half_size[2]),
        pos +  Vector3(-half_size[0],  half_size[1],  half_size[2]),
        pos +  Vector3(-half_size[0],  half_size[1], -half_size[2]),
        pos +  Vector3( half_size[0], -half_size[1], -half_size[2]),
...
void uiWindow::AddPlane(const Vector3& pos, const Vector3& size, const Vector3& normal, const Vector3b& col){
  ...
  if( normal[0] >= normal[1] && normal[0] >= normal[2]){ // X軸に近い直線
      e1 = {normal[1], normal[0], normal[2]}; e2 = {normal[2], normal[1], normal[0]};
  }else if(normal[1] >= normal[0] && normal[1] >= normal[2]){
      e1 = {normal[1], normal[0], normal[2]}; e2 = {normal[0], normal[2], normal[1]};
  }else{
      e1 = {normal[2], normal[1], normal[0]}; e2 = {normal[0], normal[2], normal[1]};
  }
  e1 = dp.cross(e1); e1 = e1.normalize() / 2.0f;
  e2 = dp.cross(e1); e2 = e2.normalize() / 2.0f;
  const Vector3 p[4] = {
    pos+(-e1-e2)*size[0],
    pos+(e1-e2)*size[1],
    pos+(e1+e2)*size[1],
...
Vector2d uiWindow::get_text_size(const std::string &str, float size)const{
  ...
  for(int i=0; i<u32str.size(); i++){
      ...
      whole_size = {
          std::max<int>(whole_size[0], x),
          std::max<int>(whole_size[1], y_tmp)
      };
  }
  return whole_size;
}
...
void uiWindow::AddStringBalloon(const std::string &str, const Vector3 &pos, const float size, const Vector3b &col, const Vector3b &line){
  ...
  const auto to = Vector2d{ 
    (int) (projected[0] * this->size[0] / 2.0f / projected[3] + this->size[0] / 2.0f), 
    (int) (projected[1] * this->size[1] / 2.0f / projected[3] + this->size[1] / 2.0f)
  };
...
void uiWindow::__AddPointSizeZero2D(const Vector2d &pos, const Vector3b &col){
    const auto text_renderer = getTextRendererPtr();
    vertices_ui.push_back(std::move(VertexUI(pos, col, text_renderer->TexUvWhitePixel)));
}
...
Vector2d uiWindow::AddString2D(const std::string &str, const Vector2d &pos, const float size, const Vector3b &col, const int xlim){
    ...
    for(int i=0; i<u32str.size(); i++){
        ...
        whole_size = {
            std::max(whole_size[0], x - pos[0]),
            std::max(whole_size[1], y_tmp - pos[1])
        };
    }
    return whole_size;
}
