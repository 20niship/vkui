...
bool vkRender::isDeviceSuitable(const vk::PhysicalDevice& device) {
    ...
    if (extensionsSupported) {
        SwapChainSupportDetails swapChainSupport = Engine::getContextPtr()->windows[0]->get_renderer()->querySwapChainSupport(device);
...
void vkWndRender::createCommandBuffers(const vkUI::Engine::DrawData* dd) {
  auto device = get_device();
...
void vkWndRender::draw(vkUI::Engine::DrawData* dd) {
...

  {
    ...
    const vk::DeviceSize bufferSize = sizeof(vkUI::Engine::DrawData::UniformData);
    ...
    const vk::DeviceSize bufferSize2 = sizeof(vkUI::Engine::DrawData::UniformData);
    uniformBuffer.copyData((void *)&dd->uniform_data, bufferSize2);
  }
  if(verbose) {
